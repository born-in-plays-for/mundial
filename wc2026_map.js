...
fetch('./wc2026_fifa_rank.json').then(r => r.json()).then(d => {
  _fifaData = d;
  app.fifaRank = Object.fromEntries(
    d.rankings.flatMap(({id, rank}) => { const n = QUALIFIED_NAMES[id]; return n ? [[n, rank]] : []; })
  );
  if (document.getElementById('tab-fifa')?.classList.contains('active')) _renderFifa();
}).catch(() => {});
...
app.pop      = rawData.pop;
app.fifaRank = {};  // populated by wc2026_fifa_rank.json fetch below
