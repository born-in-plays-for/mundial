...
| `js/map-container.js` | ES module — `<world-map>` Web Component, `mapContainerTemplate()` (shared `#map-container` inner DOM), `wireMapHeightDrag()` (shared drag-to-resize/dblclick-to-reset), projection/zoom, choropleth painting (`choroFill`, `paintChoropleth`), arc drawing, flag/centroid helpers (shared by `index.js`, `chains/wc2026_chain_longest.html`, insights pages) |
