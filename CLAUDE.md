...
**Grouping + aggregate sort (`control_sidebar.js`)**: `_buildGroups()` pairs each filtered/visible team with its `app.matchInfoByIso2` opponent if that opponent is itself visible (category filters can hide one side, leaving the other a lone row); a team with no pairing data at all is also a lone row. `_groupCompare()` then sorts groups by `_aggregateValue()` summed across members — deliberately **not** the same per-item value `elo`'s own team-display comparator uses (`rank`, an ordinal position, meaningless to add across two teams): match-display sums `pts` (the actual Elo rating, a cardinal quantity) instead. `pop`/`delta` sum their own already-additive values; `alpha` has no numeric aggregate, so groups compare by their alphabetically-first member's name instead. The two members within a couple are ordered by that same per-member value (stronger/alphabetically-first leads) — arbitrary but stable; nothing downstream depends on which one leads.
