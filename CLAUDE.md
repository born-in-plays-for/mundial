...
| File | Purpose |
|---|---|
| `index.html` | Main map page and the site's entry point (Bootstrap 5, loads JS + JSON via ES module), carries OG meta tags |
| `inde0.html` | The old redirect-stub `index.html`, kept for reference only — not linked from anywhere and not served; superseded when the map page (formerly `wc2026_map.html`) was renamed directly to `index.html`, removing the redirect hop |
| `wc2026_live.html` | Live game tracking page (Socket.IO, backend-dependent) |
| `js/index.js` | ES module — D3 rendering, zoom, tooltips (lit-html), filter sidebar, Elo tab, dim/arc logic |
...
| `js/map-container.js` | ES module — `<world-map>` Web Component, `mapContainerTemplate()` (shared `#map-container` inner DOM), `wireMapHeightDrag()` (shared drag-to-resize/dblclick-to-reset), projection/zoom, choropleth painting (`choroFill`, `paintChoropleth`), arc drawing, flag/centroid helpers (shared by `index.js`, `chains/index.html`, insights pages) |
...
| `js/diverging-debug.js` | ES module — `initDivergingDebug()`: renders the `#diverging-debug` dev panel's own markup (into `index.html`'s `#diverging-debug-slot`) and wires it to `diverging-scale.js`'s live-tunable params; panel only visible with a `?debug` URL param |
...
| `geo/` | Static frontend-only geometry, never touched by the `mundial-build` pipeline: `cape-verde.geojson`, `curacao.geojson` (both absent from the 110m world-atlas topojson, drawn as standalone insets — see "Small island countries" below; source: [geoBoundaries](https://www.geoboundaries.org/) ADM0 boundaries, ring winding manually corrected for d3-geo's convention — see commits `825728a`/`e25c4b6`), `uk-nations.geojson` (the 4 UK home nations' polygons — see CLAUDE.md's own "UK home nations" section; source: Natural Earth 1:10m Admin 0 – Map Subunits (naturalearthdata.com), filtered to the 4 UK subunits — confirmed by matching NE_ID/WOE_ID/WIKIDATAID against the upstream dataset — moved here from the `data/` submodule, where it always was pipeline-untouched static geometry anyway), `france-departments.geojson` (the 101 French départements, incl. overseas — used by `insights/france.html`'s choropleth; source: [Kaosamami/france-geojson](https://github.com/Kaosamami/france-geojson) (GPL-3.0, itself derived from OpenStreetMap Nominatim data) — vendored and simplified locally via `npx mapshaper <raw> -simplify 10% -o geo/france-departments.geojson format=geojson reverse-winding` rather than fetched live from `raw.githubusercontent.com` on every page load; **`reverse-winding` is required** — mapshaper's own GeoJSON writer flips ring winding order by default (confirmed even with a no-op passthrough, no simplification at all), and the reversed winding broke `d3.geoConicConformal`'s rendering for some départements (e.g. Ain/`01` blew up to a coordinate of ~8.25M instead of the expected ~600), so `reverse-winding` cancels that flip back out — same class of d3-geo winding gotcha as `cape-verde.geojson`/`curacao.geojson` above) |
...
| `insights/france.html` | Choropleth of French départements by WC2026 players born there (any nationality) — D3 `geoConicConformal` projection, hover tooltip per département (Wikipedia-linked player names via `pid`, export arrow, coach badge), zoom/pan, "Petite couronne" (Paris + inner suburbs) inset. Restored from the same cleanup, rebuilt against today's conventions rather than byte-for-byte: département assignment is a live point-in-polygon join against `data/v2/birthplace.json`'s geocoded coordinates (not the original's hand-maintained name→département list, which matched by name — exactly what this app's pid convention exists to avoid — and went stale as squads changed); overseas départements (971/972/973/974/976) need a small real-world-bounding-box fallback for that join, since `geo/france-departments.geojson` repositions them next to mainland France purely for display. Page content is French-only (matching the original); only the navbar's own link label is a real `T`/i18n string like every other nav item. |
...
`js/players_sidebar.js`), `control-sidebar-test.html`, and `insights/status.html` /
`heat-map.html` (and its own KDE module, `js/kde_layer.js`) / `taxonomy.html` have all been
removed — their functionality is superseded by `index.html`'s own
...
(`insights/france.html` was in this same batch — it's been restored, see the file table above.)

---

## Frontend stack

All dependencies served from a single CDN — **jsDelivr** (`cdn.jsdelivr.net/npm/`) — with one exception, Bootstrap's own CSS (see its own row below):

| Package | Version | Purpose |
|---|---|---|
| `d3` | 7.8.5 | Map rendering, zoom, D3 data joins |
| `topojson-client` | 3.1.0 | GeoJSON feature/mesh extraction |
| `bootstrap` | 5.3.3 | Responsive layout utilities — CSS is a locally-built, site-themed compile (`css/bootstrap-custom.min.css`, source `css/bootstrap-custom.scss`; `npm run build:css` to rebuild), used by every page (`index.html`, `chains/index.html`, `insights/discipline.html`, `wc2026_live.html`); the JS bundle stays on the CDN, unmodified |
...
| `world-atlas` | 2 | 110m TopoJSON world map fetched at runtime by `index.js` |

`index.js` is loaded as `<script type="module">` so it can use the `import` statement at the top.
...
**`#map-container`'s inner DOM is shared, the div itself is not.** `index.html` and `chains/index.html` both keep a real, empty `<div id="map-container" class="flex-column container-xxl px-0 mt-0 position-relative"></div>` statically in their own HTML, then each page's own module script calls `render(mapContainerTemplate({ basePath, withRangeFilter }), document.getElementById('map-container'))` (`js/map-container.js`) — right after its imports, before anything queries `world-map`/`#map-frame`/etc. — to fill in everything below it (map frame, zoom controls, grip, legend). This eliminated ~50 lines of drifting hand-duplication between the two files down to two params (`basePath`: `''` vs `'../'`; `withRangeFilter`: only `index.html` has `#legend-filter-device`). The outer div deliberately stays real markup rather than becoming a generic JS-rendered placeholder too: `<mundial-auth-bar>`'s own `_offsetSibling()` (`js/auth-bar.js`) reads its `nextElementSibling`'s **computed `position`** to decide whether to set `top` or `marginTop` on it, and on the chain page (no `#page-header` between them) that sibling is `#map-container` itself — wrapping it in an extra `position:static` placeholder div would make that check see the wrapper instead and offset the wrong element.

**`#map-footer` drag-to-resize is shared too** — `wireMapHeightDrag({ storageKey, onDrag, onSettle, skipRestore })` (`js/map-container.js`), called once per page right where the old hand-duplicated block used to be. Dragging the bar sets an explicit inline px height on `#map` (letterboxing, since `preserveAspectRatio` is `xMidYMid meet`), capped at `mapNaturalHeight()` (the full-width "contain" point — past it there's only more letterbox to reveal) so it can't be dragged into dead space; double-clicking eases it back to that natural height via a `.3s` `height` transition, polled every frame via `requestAnimationFrame` so a caller's own padding-sync tracks the animating edge. `onDrag`/`onSettle` are passed in rather than assumed, since "keep body padding in sync" means something different per page: `index.html` passes its own `_syncMapHeight` (rAF-deferred, for the high-frequency `pointermove` case) and `_syncPaddingTop` (direct, since the dblclick reset's own poll loop is already rAF-driven); the chain page passes its simpler `_syncBodyPadding` for both, having no landscape-mobile concept to complicate it. Returns `{ mapNaturalHeight, clampMapHeight }` — both callers need `clampMapHeight` again on window resize (a previously-set explicit height may now overshoot a narrower natural height). The chain page's own copy of this had drifted behind `index.js`'s (no dblclick-to-reset at all, and no natural-height cap on the drag itself — letting it be dragged into pure empty letterbox with nothing left to reveal) before being unified here.
...
- **Colors/easing** live in `_divergingParams` (`getDivergingParams()`/`setDivergingParams()`),
  not baked-in constants — live-tunable via the `#diverging-debug` panel, both rendered
  (into `index.html`'s `#diverging-debug-slot`) and wired by `js/diverging-debug.js`'s
  `initDivergingDebug()`; hidden by default via inline `display:none` on the
...
**Export/native/import filter** (`control_sidebar.js`'s `csb-native-table`, 3 buttons below the sort/display tables, header labeled "view" — it reshapes *how* the already-selected countries' players are shown, not which countries are selected) — **always enabled**, regardless of which bottom tab is active (`sidebar.setPlayersTabActive` still tracks `#tab-players`' active/inactive state for `.csb-sort-dir`'s own dual meaning, above, but no longer disables these checkboxes when the tab isn't active — its effect is meaningful to set up in advance of switching to that tab, not just while looking at it). `_focusedPlayers` tags each player with the role(s) that earned them a place in the union relative to the *whole* focus set — export (born in a set-country, plays for any other, even another set-country), native (born and plays for the same set-country), import (born **outside the whole set**, plays for a set-country) — deliberately excluding a cross-set move's destination side from import (already counted as the source team's own export), which makes the three a clean partition: verified against real data, sums match exactly (players and coaches) in both single-team and full-48-team views. `_currentPlayerSet(focusIds)` layers this filter (inclusive-OR over `_roles`, read via `sidebar.playersFilterChecked`) on top of `_focusedPlayers`/`_visiblePlayerEntries`, shared by the table and the map's birth-city dots (`_updatePlayerCityDots`/`_buildCityRecords`, also reading `_currentPlayerSet`) so they can't drift apart. Toggling it re-renders the table immediately (`callbacks.onPlayersFilterChange`).
...
**Click handling**: the visible `.elo-item.elo-item--qualified` pill inside the active carousel slide has its own click listener (toggles the `qie/qi/qe/q` show/hide checkboxes, same as `?show=QB` — independent of carousel position); the carousel's own click handler `stopPropagation()`s for clicks on `.carousel-control-prev/-next/.carousel-indicators` so paging never triggers that toggle.
...
`app.matchInfoByIso2` is built once per page load (`index.js`) after `statusByIso2` and `data/fixtures.json` both load, keyed by ELIM_ROUNDS index so it lines up 1:1 with the carousel's own `_stage` (`CAROUSEL_STAGES[p] === ELIM_ROUNDS[p]` for p 1..5).
...
**`mapData.natives` structure** (`data/v2/map.json`, used by `index.js`, not by the live-game page)
...
- `chains/chain_video.html` — 1920×1080 animated HTML
...
| File | Purpose |
|---|---|
| `index.html` | Snake renderer (formerly `wc2026_chain_longest.html`) — loads any chain JSON via `?data=<file>`, default: `subgraphs/longest_both.json` |
| `chain_render.js` | Shared chain SVG renderer (ES module, used by the map page + this one) |
| `chain_loop.json` | Bosnia ⇄ Croatia mutual cycle (bidirectional export edge case) |
