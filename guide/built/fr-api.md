...
Reflète le carrousel d'étapes du panneau de filtre (Toute la compétition → Phase de groupes → Seizièmes de finale → Huitièmes de finale → Quarts de finale → Demi-finales → Finale → Vainqueur) — sauf **Toute la compétition**, la première position du carrousel, qui n'a pas de valeur `?stage=` propre ; partager un lien depuis cette position retombe sur `?stage=group`, à un clic de carrousel de là.
