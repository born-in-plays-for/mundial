...
Spiegelt das Phasen-Karussell im Filterpanel wider (Gesamter Wettbewerb → Gruppenphase → Sechzehntelfinale → Achtelfinale → Viertelfinale → Halbfinale → Finale → Sieger) — außer **Gesamter Wettbewerb**, der ersten Position des Karussells, die keinen eigenen `?stage=`-Wert hat; das Teilen eines Links von dieser Position aus fällt auf `?stage=group` zurück, einen Karussell-Klick entfernt.
