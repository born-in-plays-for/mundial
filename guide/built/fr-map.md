...
L'état du filtre et du tri peut aussi être configuré directement depuis l'URL — `?sort=`, `?dir=`, `?stage=`, `?show=`, `?fifaconf=`. Ajoutez `?explain` à n'importe quelle URL pour ouvrir un panneau résumant les réglages actuels du panneau — voir *« ?explain » — inspecter la configuration actuelle* dans le [guide de la page Pays](?guide=countries) pour le détail exact de ce qu'il affiche et pourquoi. La référence complète avec tous les codes de cellule, alias de groupe et exemples s'y trouve aussi.
