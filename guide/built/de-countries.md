...
Spiegelt das Phasen-Karussell im Filterpanel wider (Gruppenphase → Sechzehntelfinale → Achtelfinale → Viertelfinale → Halbfinale → Finale → Sieger). Jede Position filtert qualifizierte Länder auf jene, die diese Phase „erreicht" haben — zu Beginn noch im Turnier, oder bereits Sieger. Nicht qualifizierte Exporteurländer (`ef`/`en`-Zellen) sind nicht betroffen, ebenso wie nicht exportierende, nicht qualifizierte Länder (`of`/`on`-Zellen) — keines von beiden hat eine Turnierposition zu „erreichen".
...
- `?stage=r32&show=exp` → qualifizierte Exporteure, die das Sechzehntelfinale erreicht haben, plus alle nicht qualifizierten Exporteure (unabhängig von der Phase)
- `?stage` hat keinen Effekt auf nicht qualifizierte Zeilen (`ef`/`en`/`of`/`on`) — keine von ihnen hat eine Turnierposition zu erreichen
...
?stage=r32&show=exp           Exporteur-Spalte; qualifizierte Exporteure gefiltert auf das Sechzehntelfinale, nicht qualifizierte Exporteure nicht betroffen.
