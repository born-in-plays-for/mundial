...
### <img class="gp-icon" src="images/solar_linear/user-circle-svgrepo-com.svg" alt=""> Alle Spieler

Klicken Sie auf das Personensymbol desselben Tabs, ohne dass ein Land ausgewählt ist, um stattdessen jeden Spieler und Trainer aller 48 qualifizierten Kader in einer einzigen Tabelle aufzulisten, gefiltert nach den auf der Karte gerade sichtbaren Mannschaften.

Ein Schalter **Bubbles / Intensity** über der Tabelle ändert, wie ihre Geburtsstädte auf der Karte dargestellt werden:

![Geburtsstadt-Bubbles](screenshots/bubbles.png)

**Bubbles** (Standard) setzt einen Punkt pro einzigartiger Geburtsstadt unter den aufgelisteten Spielern — größer, wenn mehr Spieler sich eine Stadt teilen. Fahren Sie mit der Maus über einen Punkt für Stadtname und Spielerzahl.

![Wärmekarte der Talentproduktion](screenshots/heatmap.png)

**Intensity** färbt stattdessen die gesamte Karte mit einer glatten Wärmeoberfläche, die eine andere Frage beantwortet: Produziert dieser Teil der Welt *mehr* WM-Talente, als seine Bevölkerung allein vermuten ließe? Violett bedeutet deutlich über dem globalen Durchschnitt, Türkis darunter, blass etwa das Erwartete. Es ist nach Bevölkerung normalisiert, keine reine Spielerzahl-Karte — ein Ort braucht eine ungewöhnlich hohe Spielerzahl *für seine Bevölkerung*, um aufzufallen, weshalb das Ergebnis nicht einfach von den größten Städten dominiert wird. Die Länderfärbung ist ausgeblendet, solange diese Ansicht aktiv ist, damit der Verlauf klar lesbar bleibt, und die Grenzen werden zum Ausgleich etwas kräftiger gezeichnet; die Legende wechselt zur selben Skala, umgerechnet in einfache „×N der erwarteten Rate"-Marken.
