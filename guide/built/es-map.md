...
- <span style="color:rgba(23,23,21,.75)">●</span> **nativo** (nacido y seleccionado aquí) / <span style="color:#dc2626">◀</span> **juega por** (nacido en otro lugar, seleccionado aquí) / <span style="color:#1d4ed8">▶</span> **nacido aquí** (nacido aquí, seleccionado en otro lugar): qué rol le dio a un jugador su lugar en la tabla.
- **J** jugador / **E** entrenador: qué tipo de persona se muestra.

Cada casilla está marcada por defecto (mostrando a todos); desmarque una para ocultar ese grupo. Actualmente solo activo dentro de *La tabla de jugadores*, más abajo — las casillas también se muestran en otros lugares, pero allí siguen sin efecto, por ahora.

## Filtrar

La matriz cruza dos **columnas** (tiene jugadores nacidos aquí que juegan en otro lugar, o no) con cuatro **filas** en dos grupos:

- **Clasificados** — divididos según si la plantilla incluye jugadores nacidos en otro lugar
...
El fondo de la píldora es a su vez un degradado rojo (juega por, nacido en otro lugar) → blanco (nativo) → azul (nacido aquí, juega en otro lugar) — cuanto más ancha la banda de un color, mayor la proporción de ese grupo en la plantilla total del país.
...
Predominantemente azul — 81 jugadores nacidos aquí juegan hoy en otro lugar, frente a solo 3 llegados de otro lugar para jugar aquí.
...
Una banda roja visible junto al azul — una mezcla más equilibrada: 11 jugadores nacidos aquí juegan en otro lugar, 7 llegaron de otro lugar para jugar aquí.
...
Cada país se colorea según su balance neto de talento — jugadores nacidos aquí que juegan en otro lugar, menos jugadores nacidos en otro lugar que juegan aquí (ver *La leyenda*, abajo); los jugadores nativos que siguen jugando para su propio país no cuentan en ninguna dirección. Cuanto más marcado sea ese balance, en cualquier dirección, más oscuro el tono; un país cercano al equilibrio neutro aparece pálido. Los países sin datos para esa métrica aparecen en un tono claro neutro.
...
El mapa colorea cada país según su balance neto de talento — jugadores nacidos aquí que juegan en otro lugar, menos jugadores nacidos en otro lugar que juegan aquí; los jugadores nativos que siguen jugando para su propio país no cuentan en ninguna dirección. Los países que envían más jugadores de los que reciben aparecen en un color, los que reciben más de los que envían en otro, a cada lado de un punto neutro central.

La barra de color en la parte inferior de la cabecera se lee de izquierda a derecha como una recta numérica — extremo negativo, cero neutro en el centro, extremo positivo — con una marca de referencia en cada extremo y en el centro, además de una pequeña marca propia para cada país real, para que puedas ver dónde se agrupan realmente los países en lugar de suponer que el degradado uniforme significa una distribución homogénea. Un único punto independiente se sitúa más allá del extremo positivo para **Francia**, cuyos jugadores nacidos aquí y hoy en otro lugar superan con creces a los de cualquier otro país — suficientemente fuera de escala (36 puntos por encima del siguiente país) como para merecer su propia marca en lugar de una marca más en la barra:
...
- **Países de nacimiento**: número de jugadores nacidos allí y mejores jugadores, cada uno con la bandera del país por el que juegan
- **Países clasificados que también reclutan**: una columna derecha añade a los jugadores llegados de otro lugar
...
La misma lista de insignias, esta vez limitada a los 48 países **clasificados**, con un pequeño carrusel encima que recorre ocho posiciones: **Toda la competición → Fase de grupos → Dieciseisavos de final → Octavos de final → Cuartos de final → Semifinales → Final → Campeón**.

- Usa las flechas ‹ › o desliza a izquierda/derecha en pantallas táctiles para cambiar de fase.
- **Toda la competición** muestra los 48 países clasificados, sin filtrar. Cada otra posición filtra los países clasificados a los que han "alcanzado" esa fase — todavía en el torneo al principio, o ya campeones.
